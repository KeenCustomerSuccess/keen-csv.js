...
class KeenCSV {
  ...
  columnIsFiltered(header) {
    return Array.isArray(this.options.filteredColumns) &&
           ...
           this.options.filteredColumns.includes(header)
  }
...
/**
 * KeenCSV.convert - Convenience function. Allows for inline csv generation,
 ...
 * @return {String}   The CSV you're looking for
 */
KeenCSV.convert = function(response, options) {
  let keenToCSV = new KeenCSV(response, options);
  return keenToCSV.generateCSV();
}


/**
 * KeenCSV.convertWithOptions - Convenience function specifically for
 * promises. Allows for configuring KeenCSV as a link in a promise chain:
 *    keenAnalysis
 *      .query(blah)
 ...
 *  nestedDelimiter:    Nested hash keys are flattened with this
 *  filteredColumns:    An array of strings to disinclude from the csv
 * @return {Function}   A wrapper around KeenCSV.convert
 */
KeenCSV.convertWithOptions = function(options) {
  return function(response) {
    return KeenCSV.convert(response, options);
  }
}

module.exports = KeenCSV
